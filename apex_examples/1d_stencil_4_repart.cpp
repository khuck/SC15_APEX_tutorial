...
static boost::uint64_t global_elapsed = 0;
...
double get_idle_rate() {
    if (!counters_initialized) {
        // save the running totals across each measurement
        static double previous_idle = 0.0;
        static double previous_total = 0.0;
        //static double previous_non_idle = 0.0;
        // get the current values
        const std::string idle_time(APEX_IDLE_TIME);
        const std::string idle_rate(APEX_IDLE_RATE);
        apex_profile * pt = apex::get_profile(idle_time);
        apex_profile * pr = apex::get_profile(idle_rate);
        if (pt != NULL && pr != NULL) {
          // we don't have non-idle or total time available, so compute it
          // rate = idle / total, so total = idle / rate
          double total = pt->accumulated / pr->accumulated;
          //double non_idle = total - pt->accumulated;
          double last_total = total - previous_total;
          double last_idle  = pt->accumulated - previous_idle;
          double last_idle_rate = last_idle / last_total;
          std::cerr << "idle rate: " << last_idle_rate;
          std::cerr << ", idle time: " << last_idle << std::endl;
          previous_total = total;
          previous_idle = pt->accumulated;
          return last_idle;
        }
        return 0.0;
    }
